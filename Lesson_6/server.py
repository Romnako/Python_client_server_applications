    ...
    while True:
        ...
        if client_name == '':
            if data_in['action'] == 'presence' and data_in['user']['account_name'] != '':
                ...
                jim.RESPONSE['response'], jim.RESPONSE['allert'] = jim.SERV_RESP[0]
                print(f'{data_in["time"]} - {data_in["user"]["account_name"]}:{data_in["user"]["status"]}')
            else:
                jim.RESPONSE['response'], jim.RESPONSE['allert'] = jim.SERV_RESP[1]

        if client_name != '' and data_in['action'] =='msg':
            print(f'{data_in["time"]} - {client_name}:{data_in["massage"]}')
