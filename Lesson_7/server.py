...
def mainloop():
    ...
    sock = chat.get_server_socket(namespase.addr, namespase.port)
    ...
    while True:
        ...
        try:
            ...
            print(info)
            logger.info(info)
            clients_info = {'name': '', 'addr': client_addr, 'in_messages': []}
            clients.append(client)
            clients_info[client] = clients_info
        finally:
            r = []
            w = []
            try:
            ...
            for s_client in r:
                ...
                if clients_info[s_client]['name'] =='':
                    if data_in['action'] == 'presence' and data_in['user']['account_name'] != '':
                        ...
                        jim.RESPONSE['responce'], jim.RESPONSE['alert'] = jim.SERV_RESP[1]
            ...
            for s_client in w:
                if 'data_out' in clients_info[s_client]:
                    data_out = clients_info[s_client]
